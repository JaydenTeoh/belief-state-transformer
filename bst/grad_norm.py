...
from typing import Optional, Union, List, Dict, Tuple, NamedTuple
        ...
        **backward_kwargs
        ...
        if self.has_restoring_force:
            if not self.initted.item():
                initial_losses = maybe_distributed_mean(losses).detach()
                self.initial_losses.copy_(initial_losses)
                self.initted.copy_(True)

            elif self.initial_losses_decay < 1.:
                meaned_losses = maybe_distributed_mean(losses).detach().clone()
